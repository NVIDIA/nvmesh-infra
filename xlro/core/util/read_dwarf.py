...
logger = IDAdapter(logging.getLogger('read_dwarf'))
...
def resolve_all_types(parsed, fltr=None):
    ...
    for entry in list(parsed.values()):
...
def resolve_all_arglists(parsed, resolved_types):
    ...
    for entry in parsed:
        if entry.get("type") == "DW_TAG_subprogram":
            resolved[entry["name"]] = {
                "arglist": [(resolved_types[arg["datatype"]], arg.get("name", "")) for arg in entry["children"] if "datatype" in arg],
                "restype": resolved_types.get(entry.get("datatype", ""), None)
            }
    return resolved
...
def resolve_dwarf_only(so, fltr=None):
    ...
    types = Bunch(**{t.name: t.ctype for t in list(resolved.values())})

    # Turn enums into a bunch
    enums = Bunch()
    for dt in list(resolved.values()):
        if dt.values:
...
def resolve(so):
    ...
    funcs = Bunch()
    for func, args in list(arglists.items()):
        try:
            ...
            if not func.endswith('_') and not func.startswith('__'):
                logger.debug("Error importing {0}. Static? Probably can ignore this message. {1}".format(func, str(e)))
            continue
...
def main():
    ...
    import cProfile
    # Main for tests only
    cProfile.run("types, enums, parsed, resolved = resolve_dwarf_only(\"./nvmeib_common.ko\", 'nvmeib_ec_journal_md_cache.*')")
    IPython.embed()
    return 0
